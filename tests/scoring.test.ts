...
import {
  ...
  getHealthScoreGrade,
} from '../packages/scoring/src/site-health';
import {
  calculateKeywordDifficulty,
  getKdDifficultyLabel,
} from '../packages/scoring/src/keyword-difficulty';
import { calculateDomainRating } from '../packages/scoring/src/domain-rating';
import { calculateSpamScore } from '../packages/scoring/src/spam-score';
import { analyzeSentiment } from '../packages/scoring/src/sentiment';
import { auditNapConsistency } from '../packages/scoring/src/nap-audit';
...
test('auditNapConsistency flags address and phone mismatches', () => {
  ...
  const listings = [
    ...
    },
    {
      ...
      phone: '555-9999', // phone and address discrepancy
      website: 'https://rivue.io',
    },
  ];
  ...
  assert.strictEqual(audit.discrepancies.length, 2);
  assert.ok(audit.napScore < 100 && audit.napScore >= 75);
});
